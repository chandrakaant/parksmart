...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
            ...
            throws IOException, ServletException
    {
        ...
        if( header == null || !header.startsWith(TokenConstant.TOKEN_PREFIX) )
        {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request )
    {
        ...
        if( token != null )
        {
            ...
            String role = Jwts.parser().setSigningKey(TokenConstant.SECRET)
                    .parseClaimsJws(token.replace(TokenConstant.TOKEN_PREFIX, "")).getBody().get("role").toString();

            Integer userId = (Integer) Jwts.parser().setSigningKey(TokenConstant.SECRET)
                    .parseClaimsJws(token.replace(TokenConstant.TOKEN_PREFIX, "")).getBody().get("userId");
